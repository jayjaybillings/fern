...
if (plotJavaCompare == 1)
    javaOutFile = fopen('fern/data/Y_3alpha.out');
...
Z = zeros(1, numSpecies);
N = zeros(1, numSpecies);
X = zeros(1, numSpecies);
massNum = zeros(1,numSpecies);
sumX = 0; %initialize
for i = 1:numSpecies
    %get initial abundances
    y(i) = str2double(speciesHeader{i}{5});
    %get proton number
    Z(i) = str2double(speciesHeader{i}{3});
    %get neutron number
    N(i) = str2double(speciesHeader{i}{4});
    %sum baryon number
    massNum(i) = Z(i)+N(i);
end
%parse Reaction file
...
while t < tmax
    ...
    if(useOldTimestepper == 0)
    %if sumX deviates from 1, repeat previous timestep. 
        if(((numeq-1)/numRG < .5 || numAsy/numSpecies < .5))
            while(sumX > 2 && numTimesteps > 1)
                dt = dt*.01;
                y = updatePopulations(FplusSum, FminusSum, y, yzero, dydt, numSpecies, dt);
                sumX = NDreduceSum(X, numSpecies, massNum, y);
            end
        end
    end
    ...
    FminusSumOld = zeros(1,numSpecies);
    FplusSumOld = zeros(1,numSpecies);
    yzero = y;
    L = zeros(numSpecies);
    ...
    %save previous sumX for conservation of baryon number
    sumXLast = NDreduceSum(X, numSpecies, massNum, y);
    ...
    for i = 1:numRG
        ...
        if RGclassbyRG(i) == 1
          ...
          if (PE_val_a < PETol && PE_val_b < PETol)
            pEquilbyRG(i)  = 1;
          end
        elseif RGclassbyRG(i) == 2
          ...
          if(PE_val_a < PETol && PE_val_b < PETol && PE_val_c < PETol)
            pEquilbyRG(i) = 1;
          end
        elseif RGclassbyRG(i) == 3
          ...
          if(PE_val_a < PETol && PE_val_b < PETol && PE_val_c < PETol && PE_val_d < PETol)
            pEquilbyRG(i) = 1;
          end
...
%         
          ...
          if(PE_val_a < PETol && PE_val_b < PETol && PE_val_c < PETol && PE_val_d < PETol && PE_val_e < PETol)
            pEquilbyRG(i) = 1;
          end
        end
    end
...
%     RGrFlux
    ...
    for i = 1:numSpecies
        ...
        for j = startingFplusIndex:endingFplusIndex
            ...
            if(pEquilOn == 1 && newCalc == 1)
                FplusSum(i) = FplusSum(i) + pEquilFac(FplusReacs(j))*FplusFac(j)*Flux(FplusReacs(j));
            else
...
% % %                %Add its flux/y(m) to L(i,m).
        ...
        for j = startingFminusIndex:endingFminusIndex
            ...
            if(pEquilOn == 1 && newCalc == 1)
                FminusSum(i) = FminusSum(i) + pEquilFac(FminusReacs(j))*FminusFac(j)*Flux(FminusReacs(j));
            else
...
% -----------BEGIN OLD TIMESTEPPING ALGORITHM-------------
if(useOldTimestepper)
...
%         if ((((FplusSumOld(i)/FminusSumOld(i) > 1-AsyTol) && (FplusSumOld(i)/FminusSumOld(i) < 1+AsyTol) && checkAsy == 1) && (FplusSum(i) ~= 0 && FminusSum(i) ~= 0)) && y(i) ~= 0)
%         if ((((FplusSum(i)/FminusSum(i) > 1-AsyTol) && (FplusSum(i)/FminusSum(i) < 1+AsyTol) && checkAsy == 1) && (FplusSum(i) ~= 0 && FminusSum(i) ~= 0)) && y(i) ~= 0)
%         if (abs(FplusSum(i)-FminusSum(i)) < AsyDiffTol && checkAsy == 1 && y(i) ~= 0)
    ...
    sumXLast = sumX;
    %modify timestep to ensure particle number is conserved:
    dev_sumXLast = abs(sumXLast - 1);
    dev_sumX = abs(sumX - 1);
    massChecker = abs(sumXLast-sumX);
    if (dev_sumXLast > dev_sumX && massChecker > massTol)
        dt = dt*(max((massTol/(max(massChecker,1e-16))),downbumper));
    elseif (massChecker < massTolUp)
        dt = dt*(massTol/(max(massChecker,upbumper)));
    end
...
% -----------END OLD TIMESTEPPING ALGORITHM--------------
else

   %     ----------BEGIN NEW (9.1.16) TIMESTEPPING ALGORITHM--------------
   
   
    %%%%%%     ASYMPTOTIC APPROXIMATION      %%%%%%%
    %PLACE INTO checkAsy function:
    %checkAsy with current dt, and adjust L for column and row for species
    %that satisfy, and update dt by finding the eigen values again:
    if(AsyOn)
        AsySatisfied = 0;
        ...
        dtPrev = dt;
        dtNew = 0;
        %alter dt by removal of isotopes that are in asymptotic. If
        %asymptotic increases dt, it is possible that more isotopes will
        %satisfy the asymptotic condition, so check asymptotic again and
        %update dt until the latest update of dt equals the previous update.
        while (dtPrev ~= dtNew)
            dtPrev = dtNew;
            [dtNew, R, numAsy] = dt_AsyUpdate(numSpecies, FminusSum, y, dt, AsySpecies, AsySatisfied, R, t);
        end        
        dt = dtNew;
    end
    
    %to output dt vs t for external plotting and comparisons.
    alldt(alldtcount) = dt;
    alldtcount = alldtcount + 1;
    ...
    %Update populations first using dt calculated from flux matrices.
    if (dtLast < dt)
        dt = dtLast;
    end
    y = updatePopulations(FplusSum, FminusSum, y, yzero, dydt, numSpecies, dt);
...
%   -------------
    %Now check conservation of particle number. If there's a deviation,
    %calculate the abundances once again with this modified dt.
    
    %Calculate sumX, and check how much it deviates from 1.
    sumX = NDreduceSum(X, numSpecies, massNum, y);
    %modify timestep to ensure particle number is conserved 
    %(turn into func):
    dev_sumXLast = abs(sumXLast - 1);
...
%         dt
    ...
    %update populations again, with modified timestep to preserve particle
    %number.
    if((numeq-1)/numRG > .5 || numAsy/numSpecies > .5)
       dt = dt*1.1; 
    end
    if((numeq-1)/numRG == 1 || numAsy/numSpecies == 1)
       dt = .1*t; 
    end
    y = updatePopulations(FplusSum, FminusSum, y, yzero, dydt, numSpecies, dt);
    %preserve last dt to make buffer so dt doesn't change so drastically:
...
%     ----------END NEW (9.1.16) TIMESTEPPING ALGORITHM-------------- 
end
    ...
    %Turn into function (NDreduceSum)
    %Calculate sumX. It should have little deviation from 1
    sumX = 0;
    for i = 1:numSpecies
        sumX = sumX + X(i);
    end
    ...
    fracRGPE_plot(numTimesteps) = (numeq-1)/numRG;
    fracAsy_plot(numTimesteps) = numAsy/numSpecies;
    sumX_plot(numTimesteps) = sumX;
    t=t+dt;
    dt;
    %number of timesteps
...
for i = 1:numSpecies
    ...
    for j = 1:numTimesteps-1
        if(plotJavaCompare == 1 && ((tplot(j)/javaPlotTimes{1}(counter)) < 1.2 && (tplot(j)/javaPlotTimes{1}(counter)) > .8) && counter < length(javaPlotTimes{1}))
            tplot(j);
...
%plot abundances
if(plotJavaCompare == 1)
    %Compare Matlab and Java Abundances
    %sprintf('Len: javaTplot: %d, javaYPlot: %d, javaAbundances: %d',length(javaTPlot), length(javaYPlot), length(javaAbundances))
    figure(1)
    lh1 = loglog(javaTPlot,javaYPlot,'-',javaTPlot,javaAbundances,'--');
    axis([1e-16,tmax,1e-15,5e-1]);
    lh1;
    %Plot Abundances and Timestepping
    figure(2)
    lh2 = loglog(tplot,yplot,tplot,dtplot);
    set(lh2(numSpecies+1),'LineWidth',2);
    axis([1e-16,tmax,1e-15,5e-1]);
    lh2;
    %Plot FracRGPE and FracAsy
    figure(3)
    lh3 = semilogx(tplot,fracRGPE_plot,tplot,fracAsy_plot);
    axis([1e-16,tmax,0,1]);
    set(lh3(1),'LineWidth',2);
    set(lh3(2), 'LineWidth', 2);
    legend('fracRGPE','fracAsy');
    lh3;
    %Plot sumX
    figure(4)
    lh4 = semilogx(tplot,sumX_plot);
    axis([1e-16,tmax,0,2]);
    legend('sumX');
    lh4;
else
