...
class ContourFileDialogueFlux extends Frame {
   ...
   public ContourFileDialogueFlux (int X, int Y, int width, int height, Color fg,
      ...
      saveButton.addActionListener(new ActionListener() {
          public void actionPerformed(ActionEvent ae){
            ...
            // works by declaring ContourFrameFlus instance cd and
            // ContourPadFlux instance gp to be static and then
            // invoking the static method makePs from ContourFrameFlux
            // to in turn invoke gp.PSfile(file).  (Note the use
            // of the "ContourFileDialogueFlux.this" construction to
            // invoke the "this" reference for the outer class
            // ContourFileDialogueFlux from this anonymous inner class.)
            ...
            if ( ContourFileDialogueFlux.this.hT.getText().length() > 0 ) {
                ContourFrameFlux.makePS(ContourFileDialogueFlux.this.hT.getText());
                hide();
                dispose();
            } else {

                makeTheWarning(100,100,200,100,Color.black,
                    Color.lightGray, "Warning",
                    "You must supply a file name!",
                    true, ContourFileDialogueFlux.this);
                return;
            }
          }
