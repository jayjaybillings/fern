// -----------------------------------------------------------------------------------------------
//  Class MyFileDialogue to create generic file dialog window.
//  Variables:
...
class MyFileDialogue extends Frame {
    ...
    public MyFileDialogue(int X, int Y, int width, int height, Color fg,
        ...
        saveButton.addActionListener(new ActionListener() {
            ...
            // works by declaring SegreFrame instance cd and
            // IsotopePad instance gp to be static and then
            // invoking the static method makePS from SegreFrame
            // to in turn invoke gp.PSfile(file). (Note the use
            // of the "MyFileDialogue.this" construction to
            // invoke the "this" reference for the outer class
            // MyFileDialogue from this anonymous inner class.)
            ...
            if ( MyFileDialogue.this.hT.getText().length() > 0 ) {
                SegreFrame.makePS(MyFileDialogue.this.hT.getText());
                hide();
                dispose();
            } else {
                //MyWarning fw = new MyWarning(300,300,200,100,Color.black,
                //               Color.lightGray, " Warning",
                //               "You must supply a file name!", true);
                //fw.show();
        
                makeTheWarning(100,100,250,100,Color.black,
                                Color.lightGray, "Warning",
                                "You must supply a file name!",
                                true, MyFileDialogue.this);
