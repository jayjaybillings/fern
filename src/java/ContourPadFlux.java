// -------------------------------------------------------------------------------------------------------
//  ContourPadFlux creates a graphics Canvas for the frame on which the
...
class ContourPadFlux extends Canvas implements MouseListener, Runnable {
    ...
    double [] contourRange = new double[ContourFrameFlux.numberContours];
    Color [] contourColor = new Color[ContourFrameFlux.numberContours];
    ...
    ShowIsotopeValues sa;  // Window to show current value of plotted variable for isotope
...

    {
        ...
        boxWidth = ContourFrameFlux.SMALLBOXSIZE;    // Width for isotope boxes
        boxHeight = ContourFrameFlux.SMALLBOXSIZE;   // Height for isotope boxes
    ...
    public ContourPadFlux (String contourLegend, double [] contourRange, Color [] contourColor,
        double [][][] twa) {
        ...
        currentTime = StochasticElements.timeNow[StochasticElements.numdt -1];

        // Add MouseListener to listen for mouse clicks anywhere on the canvas
        ...
        // Set canvas size so that ScrollPane knows whether to put scrollbars on the viewport

        this.setSize(xmax+xoffset,ymax+yoffset);
        setColorIndices();
        timerString = Cvert.gg.decimalPlace(6,StochasticElements.timeNow[t]);
    }
    ...
    public void run() {
        ...
        loopFlag = ShowIsotopeFlux.cd.loopcbox[1].getState();
        ...
        while(true) {
            ...
            while(animateIt && t < ts) {
                ...
                timerString = Cvert.gg.decimalPlace(6,StochasticElements.timeNow[t]);
                ShowIsotopeFlux.cd.timeField.setText(timerString);
                boxRepainter(t);
        ...
        animator = null;
        ShowIsotopeFlux.cd.animateButton.setLabel("Animate");
    }
            ...
            for( int n=minDripN[z]; n<=Math.min(maxDripN[z],nmax); n++ ) {
                ...
                for(int t=0; t<=ts; t++) {
                    for(int k=0; k<kr; k++) {
                        tryFac = twa[z][n][t] ;
                        if(tryFac < 0){
                            isoColorIndex[z][n][t] = (byte)(kr-1);
                            break;
                        }
                        if(tryFac > contourRange[kr-1]){
                            isoColorIndex[z][n][t] = (byte)(kr-1);
                            break;
                        }
    ...
    void drawMesh(int xoff, int yoff, int w, int h, Graphics g){
        ...
        if(savingFrameNow){
            ...
            // Put time readout on graphics pad so that it will export with PS files of frames.
        ...
        if(showIsoLabels) {
            ...
            g.setColor( contourColor[currentColorIndex[0][1]] );
...
//         }
        ...
        //ContourFrameFlux.sp.setScrollPosition(0,5000);
        //repaint();
    ...
    void bkgMesh(int xoff, int yoff, int w, int h, Graphics g){
        ...
        for (int z=0; z<=zmax; z++){
            for (int n=minDripN[z]; n<=Math.min(maxDripN[z],nmax); n++){
                if( ! isPStable[z][n] ) {continue;}     // omit dripped
                g.setColor(nonSelectColor);
                if(StochasticElements.fluxColorInvert){
                    g.setColor( contourColor[10] );  // Set all boxes to max if inverted color scale 
                } else {
                    g.setColor( contourColor[0] );  // Set all boxes to min if normal color scale
                }
                g.fillRect(xoffset+n*boxWidth,yoffset+(zmax-z)*boxHeight,boxWidth,boxHeight);
                g.setColor(frameColor);
                g.drawRect(xoffset+n*boxWidth,yoffset+(zmax-z)*boxHeight,boxWidth,boxHeight);
    ...
    public void mousePressed(MouseEvent me) {
        ...
        // If ctrl or shift is down on press
        ...
        if ( me.isControlDown() || me.isShiftDown() ) {
            ...
            if(me.isShiftDown()) {
                ...
                sa = new ShowIsotopeValues (300, 150, contourLegend, contourLegend, t, twa);
                sa.setLocation(450, 450);
                sa.show();
            }
...
// -----------------------------------------------------------------------------------
    ...
    public void putContourScale(Graphics g){
        ...
        for(int i=0; i<vleg; i++) {
            g.setColor( mc.returnRGB(StochasticElements.fluxColorInvert, 
                StochasticElements.fluxColorMap, (double)(vleg-i)*inc ) );
            //g.setColor( ShowIsotopeFlux.cd.returnRGB( (double)(vleg-i)*inc ) );
        ...
        String temp = Cvert.gg.decimalPlace(4,contourRange[0]);
        g.drawString(temp,20+leftSide1,topSide+(length-1)*vspacing);

        for (int i=0; i<length-1; i++) {
            temp = Cvert.gg.decimalPlace(4,contourRange[length-1-i]);
    ...
    public void saveFrames() {
        ...
        // method defined in the class ContourFrameFlux (accessed through the static instance
        // ShowIsotopeFlux.cd).
        ...
        if( !movieDir.exists() || !movieDir.isDirectory() ){
            ...
            ShowIsotopeFlux.cd.makeTheWarning(300,300,200,120,Color.black,
                Color.lightGray, " Warning!", message, false, ShowIsotopeFlux.cd );
            return;
        } 
