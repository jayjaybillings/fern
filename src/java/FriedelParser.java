...
//  and output serialized object files containing the reaction information
//  for each isotope in a selected range.  Usage:
//
//          java FriedelParser inputFile outputFile
//
...
//  desired name of the summary output file.  For each isotope processed
//  the program also outputs a file "data/isoZ_N.ser" where Z is the proton
//  number and N is the neutron number.  These files contain serialized
//  objects of type ReactionClass1 that hold the reaction information for
//  all reactions from the library involving the isotope on the left side
//  of the equation.  The objects in these serialized files may be examined
//  using the class LoadR1, and form the basis for an object-oriented
//  library of reaction rates for network calculations.  See the class
//  ReactionClass1 for data and method structure.
//
...
public class FriedelParser {
    ...
    static int minZ, maxZ, minN, maxN;
    static ReactionClass1 [] tempInstance = new ReactionClass1[600];
    static Point [] In = new Point[3];
    static Point [] Out = new Point[4];
    ...
    public static void main(String[] args) {
        ...
        if (args.length != 2) {              // Check for fileName arguments
            System.err.println();
            System.err.println("Usage: java FriedelParser <inFile> <outFile>");
            System.err.println();
            System.exit(1);
        }
        else {                               // copy; display error messages
            try { copy(args[0], args[1]); }
            catch (IOException e) { System.err.println(e.getMessage()); }
        }

        // Parse the input data and output it.  First set the range of
        // proton numbers to parse.

        minZ=0;
        maxZ=24;

        // Process between the limits set by minZ and maxZ for proton
        // number Z, assuming all relevant values of neutron number N
        // lie between Z-10 and Z+10.
        ...
        // before stream toChar and stream to are closed to ensure
        // that all data gets output.
    ...
    static void parseBuffer(int targetZ, int targetN) {
        ...
        int numberRight;       // Number products (right side of equation)
        String reacString;     // String describing reaction
        double Qvalue;         // Q for reaction
        int reacIndex = 1;     // Reaction classification index (1-8)

        String [] tempTokens = new String [10];
        String [] reactionComponents = new String[6]; //Components of
                                                      // e.g. n + p -> d

        double [] params = new double[7];       // The 7 parameters for
                                                                // Friedel rates

        int numberReactions = 0;    // Number of reactions where this
                                                 // isotope appears on left side of
                                                 // equation in Friedel table.  Note
                                                 // that a given reaction may be
                                                 // composed of a non-resonant and
                                                 // as many as two resonant components.
                                                 // Each is counted separately.

        int [] numberEachType = new int[9]; // Array holding number of
                                                             // Reactions of each type.
                                                             // Entry 0 is total number
                                                             // (=numberReactions). The
                                                             // other 8 entries are the
                                                             // number of reactions of
                                                             // types 1-8, respectively.
        ...
        while(st.hasMoreTokens()){
            ...
            if (loopCounter <= 2) {   // Header lines case: skip
                ...
                for (int i=0; i<=numberLeft-1; i++){
                    ...
                    // If this Z and N matches target Z and N

                    if(Z==targetZ && N==targetN && !isTarg) {
                        isTarg = true;
                        numberReactions ++;
                        numberEachType[0] ++;
                        numberEachType[reacIndex] ++;
                    }
                ...
                numberRight = loopCounter-numberLeft-3;
                ...
                for (int i=numberLeft; i<=loopCounter-4; i++) {
                    ...
                    int Z = tempZN[0];
                    int N = tempZN[1];
                    Out[i-numberLeft] = new Point(Z,N);
                    int mass = Z+N;
                 }
                 ...
                 boolean reverseR = false;         // true -> reverse reac
                 boolean resonant = false;         // true -> resonant
                 boolean nonResonant = false;   // true -> nonresonant
                 ...
                 // Extract the 7 parameters from the tokens
                 // as doubles
                 ...
                 // Print 7 parameters for reaction rates and
                 // create an instance of a ReactionClass1 object
                 // in temporary array tempInstance
                 ...
                 if(isTarg) {
                     ...
                     tempInstance[numberReactions-1] =
                        new ReactionClass1(reacIndex,
                        numberLeft,numberRight,ecFlag,
                        reverseR,resonant,nonResonant,
                        reacString,refString,
                        In, Out, Qvalue, params);
                 }
        ...
        // All tokens have been parsed, so the array tempInstance now
        // holds a set of objects of type ReactionClass1 that specify
        // all possible reactions having the isotope defined by
        // Z = targetZ and N = targetN in the entrance channel.  We
        // now serialize the objects in the temporary array to a
        // disk file, so that they can be deserialized at runtime to
        // local arrays that will then hold all necessary information
        // about rates for isotope species participating in the
        // network calculation.

        if(numberReactions > 0) {

            // First copy to a new array just large enough to
            // hold all reaction objects

            ReactionClass1 [] instance = new ReactionClass1[numberReactions];
            System.arraycopy(tempInstance,0,instance,0,numberReactions);

            // Then serialize the array of reaction objects using
            // the static serializeIt method of ReactionClass1.
            // Name the serialized output files for the corresponding
            // isotope in the form "isoZ_N.ser".  For example, the
            // serialized output file of objects for all reactions
            // involving carbon-13 in the entrance channel will have
            // the name "iso6_7.ser".  Place all these files in the
            // subdirectory "data" of the current directory.  The
            // content of these serialized files may be read using the
            // class LoadR1. For example, "java LoadR1 data/iso6_7.ser"

            String fName = "data/iso" + targetZ + "_" + targetN + ".ser";
            ReactionClass1.serializeIt(fName,numberEachType,instance);

        }

        System.out.println("Z="+targetZ+" N="+targetN+
            "  numberTokens = "+numberTokens
            + "  Reaction Objects Found = "
            + numberReactions);
    ...
    */
	...
	throws IOException
    {
		...
		buffNumber ++;
            ...
            if(buffNumber > 1){
                callExit("\n*** Error in FriedelParser: Buffer size "
